...
then
  declare REACTIVE_AUDIT_HOME="$(dirname $(realpath "../$0"))"
else
...
weaver=-javaagent:$(cygwinpath "$REACTIVE_AUDIT_HOME/lib/aspectjweaver.jar")
...
extdir="-Djava.ext.dirs=$(cygwinpaths "$ext" "${REACTIVE_AUDIT_HOME}/lib")"
